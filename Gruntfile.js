...
module.exports = function (grunt) {
    ...
    grunt.initConfig({
        ...
        php: {
            dist: {
                options: {
                    port: 5000,
                    base: 'server', // folder from where the webserver will be served,
                    keepalive: true,
                }
            }
        },
        protractor: {
            ...
            e2e: { // Grunt requires at least one target to run so you can simply put 'all: {}' here too.
                options: {
                    configFile: "e2etests/protractor.conf.js", // Target-specific config file
                    args: {} // Target-specific arguments
                }
            },
        },

        ngconstant: { // Distinguish between PRODUCTION AND DEVELOPMENT
            options: {
              space: '  ',
              wrap: '"use strict";\n\n{%= __ngModule %}',
    ...
    grunt.registerTask('serve', 'Compile then start a connect web server', function (target) {
        ...
        grunt.task.run([
            'clean:server',
            'ngconstant:dev', // We are in development stage
            'wiredep',
            'concurrent:server',
            'autoprefixer:server',
            'connect:livereload',
